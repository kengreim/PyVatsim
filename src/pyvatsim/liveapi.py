from __future__ import \
    annotations  # Required for type annotations to use forward reference
...
class PilotRating(NameTable):
    ...
    @classmethod
    def from_api_json(
        cls, json_dict: dict, api: Optional[VatsimLiveAPI] = None
    ) -> PilotRating:
...
class Flightplan:
    ...
    def from_api_json(
        ...
        args["enroute_time"] = timedelta(
            hours=int(args["enroute_time"][:2]), minutes=int(args["enroute_time"][2:])
        )
        args["fuel_time"] = timedelta(
            hours=int(args["fuel_time"][:2]), minutes=int(args["fuel_time"][2:])
        )
        ...
        try:
            ...
            args = {
                ...
                "time": datetime(
                    ...
                    today.month,
                    int(m.group("time")[:2]),
                    int(m.group("time")[2:4]),
                    int(m.group("time")[4:6]),
                ),
            }
        except:
            args = {
    ...
    def is_stale(self, key="_ALL"):
        ...
        if key not in self._cache:
            ...
            return (
                ...
                or (now - self._last_update_time[key]).total_seconds() > self.ttl
            )
...
class VatsimLiveAPI:
    def __init__(
        self,
        vatsim_endpoints: VatsimEndpoints = None,
        DATA_TTL: int = 15,
        METAR_TTL: int = 60,
    ) -> None:
    ...
    def metars(
        ...
        update_mode: UpdateMode = UpdateMode.NORMAL,
    ) -> None | dict[str, Metar]:
        ...
        if fields is None:
            ...
            r = {
                k: v
                for k, v in self._metar_cache.get_cached().items()
                if k in VatsimLiveAPI.wrap_if_single(fields)
            }
            return r if len(r.keys()) > 0 else None

    def metar(
        self, field: str, update_mode: UpdateMode = UpdateMode.NORMAL
    ) -> None | Metar:
        self._update_metars_if_needed(
            update_mode=update_mode
        )  # could only request 1 filed instead of all
    ...
    def _return_list_filtered_cid_or_callsign(
        self, cache_key, cids=None, callsigns=None, update_mode=UpdateMode.NORMAL
    ):
        if cids is not None:
            ...
            def filter(v):
                return any(
                    [
                        re.search(i, getattr(v, "callsign"))
                        for i in VatsimLiveAPI.wrap_if_single(callsigns)
                    ]
                )
    ...
    def _return_single_filtered_cid_or_callsign(
        self, cache_key, cid=None, callsign=None, update_mode=UpdateMode.NORMAL
    ):
        if cid is not None:
    ...
    def pilot(
        ...
        return self._return_single_filtered_cid_or_callsign(
            "pilots", cid, callsign, update_mode
        )

    def pilots(
        self,
        cids: Optional[int | list[int]] = None,
        callsigns: Optional[str | list[str]] = None,
        update_mode: UpdateMode = UpdateMode.NORMAL,
    ) -> None | dict[int, ActivePilot]:
        return self._return_list_filtered_cid_or_callsign(
            "pilots", cids, callsigns, update_mode
        )
    ...
    def prefiled_pilot(
        ...
        return self._return_single_filtered_cid_or_callsign(
            "prefiles", cid, callsign, update_mode
        )
    ...
    def prefiled_pilots(
        ...
        callsigns: Optional[str | list[str]] = None,
        update_mode: UpdateMode = UpdateMode.NORMAL,
    ) -> None | dict[int, PrefiledPilot]:
        return self._return_list_filtered_cid_or_callsign(
            "prefiles", cids, callsigns, update_mode
        )
    ...
    def controller(
        ...
        return self._return_single_filtered_cid_or_callsign(
            "controllers", cid, callsign, update_mode
        )
    ...
    def controllers(
        ...
        callsigns: Optional[str | list[str]] = None,
        update_mode: UpdateMode = UpdateMode.NORMAL,
    ) -> None | dict[int, Controller]:
        return self._return_list_filtered_cid_or_callsign(
            "controllers", cids, callsigns, update_mode
        )
    ...
    def atises(
        ...
        return self._return_list_filtered_cid_or_callsign(
            "atis", cids, callsigns, update_mode
        )

    def pilot_ratings(
        self, update_mode: UpdateMode = UpdateMode.NORMAL
    ) -> None | dict[int, PilotRating]:
        return self._return_whole("pilot_ratings", update_mode)

    def pilot_rating(
        self, id: int, update_mode: UpdateMode = UpdateMode.NORMAL
    ) -> None | PilotRating:
        return self._return_single_exact_match("pilot_ratings", id, update_mode)

    def facilities(
        self, update_mode: UpdateMode = UpdateMode.NORMAL
    ) -> None | dict[int, Facility]:
    ...
    def controller_ratings(
        self, update_mode: UpdateMode = UpdateMode.NORMAL
    ) -> None | dict[int, Rating]:
        return self._return_whole("ratings", update_mode)

    def controller_rating(
        self, id: int, update_mode: UpdateMode = UpdateMode.NORMAL
    ) -> None | Rating:
    ...
    def servers(
        self, update_mode: UpdateMode = UpdateMode.NORMAL
    ) -> None | dict[str, Server]:
        return self._return_whole("servers", update_mode)

    def server(
        self, ident_str: str, update_mode: UpdateMode = UpdateMode.NORMAL
    ) -> None | Server:
